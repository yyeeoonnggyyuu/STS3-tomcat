...
public class AdminMemberDao {
	...
	public List<AdminMemberVo> selectAdmins() {
		...
		/*SELECT문에서 넘어온 담을수 있는 타입이 필요함 전체를 한번에 담을 수 있는건 list 또는 Map*/
		List<AdminMemberVo> adminMemberVos = new ArrayList<AdminMemberVo>();
		/*adminMemberVos 복수이기에 s */
		try {
													/*두번째 매개변수 익명클래스 한번쓰고 버리기위해서 바로 실행문*/
			adminMemberVos = jdbcTemplate.query(sql, new RowMapper<AdminMemberVo>() {

				@Override		/*RowMapper로 받은 리스트를 resultSet rs에서 rs에 저장 후 int rowNum으로 첫번째 아이디를 받고 */
				public AdminMemberVo mapRow(ResultSet rs, int rowNum) throws SQLException /*SQL에서 문제가 날경우 나타내는 문구*/{ 
					AdminMemberVo adminMemberVo = new AdminMemberVo(); 
